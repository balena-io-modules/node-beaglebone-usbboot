...
export class Message {
	...
	public identify(buff: Buffer): string {
		...
		if (ether.h_proto === ETH_TYPE_IPV4) {
			...
			if (ipv4.Protocol === IP_UDP) {
				const udp = parser.parseUdp(
					buff.slice(RNDIS_SIZE + ETHER_SIZE + IPV4_SIZE),
				);
				...
				if (dPort === TFTP_PORT) {
					const opcode =
						buff[RNDIS_SIZE + ETHER_SIZE + IPV4_SIZE + UDP_SIZE + 1];
					if (opcode === 1) {
						return 'TFTP';
		...
		if (ether.h_proto === ETH_TYPE_IPV6) {
			...
			if (ipv6.NextHeader === IPV6_HOP_BY_HOP_OPTION) {
				const ipv6Option = parser.parseIpv6Option(
					buff.slice(RNDIS_SIZE + ETHER_SIZE + IPV6_SIZE),
				);
	...
	public getBOOTPResponse(data: Buffer, serverConfig: any): Buffer {
		...
		data.copy(
			bootpBuf,
			0,
			RNDIS_SIZE + ETHER_SIZE + IPV4_SIZE + UDP_SIZE,
			MAXBUF,
		);
		...
		const eth2 = this.encoder.makeEther2(
	...
	public getBootFile(data: Buffer, serverConfig: any) {
		...
		data.copy(
			udpTFTPBuf,
			0,
			RNDIS_SIZE + ETHER_SIZE + IPV4_SIZE,
			RNDIS_SIZE + ETHER_SIZE + IPV4_SIZE + UDP_SIZE,
		);
	...
	public getTFTPData(serverConfig: any): Buffer {
		...
		serverConfig.tftp.fileData.copy(
			blockData,
			0,
			serverConfig.tftp.start,
			serverConfig.tftp.start + blockSize,
		); // Copying data to block
		serverConfig.tftp.start += blockSize; // Keep counts of bytes transferred upto
		const rndis = this.encoder.makeRNDIS(
			ETHER_SIZE + IPV4_SIZE + UDP_SIZE + TFTP_SIZE + blockSize,
		);
		const ip = this.encoder.makeIPV4(
			serverConfig.receivedARP.ip_dest,
			serverConfig.receivedARP.ip_source,
	...
	public getTFTPError(serverConfig: any): Buffer {
		...
		const rndis = this.encoder.makeRNDIS(
			ETHER_SIZE + IPV4_SIZE + UDP_SIZE + TFTP_SIZE + errorMsg.length + 1,
		);
		const ip = this.encoder.makeIPV4(
			serverConfig.receivedARP.ip_dest,
			serverConfig.receivedARP.ip_source,
